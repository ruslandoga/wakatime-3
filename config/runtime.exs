...
case System.get_env("W3_MODE", "server") do
  "server" ->
    ...
    fetch_in = fn path, env ->
      System.get_env(env) || get_in(Application.get_all_env(:w3), path) ||
    ...
    config :w3,
      ...
      http: [
        ...
        port:
          parse_integer.(
            "HTTP_PORT",
            get_in(Application.get_all_env(:w3), [:http, :port]) || "4000"
          )
      ],
      s3: [
  ...
  other ->
    raise ArgumentError, "Invalid W3_MODE: #{other}. Must be 'server' or 'compactor'."
end
